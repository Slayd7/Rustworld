...
impl Assets {
    ...
    pub fn add_actor_image(
        ...
        self.actorimages
            .insert(*id, Asset::new(SpriteBatch::new(image)));
        self.names.insert(name.to_string(), *id);
        Ok(())
    }
/*
...
impl StateManager {
    ...
    fn initialize_assets(ctx: &mut Context) -> GameResult<Assets> {
        ...
        for e in parser {
            // XML parsing. Messy, but works
            match e {
                Ok(XmlEvent::Characters(e)) => match elm {
                    Elements::name => {
                        name = e.to_string();
                    }
                    Elements::location => {
                        match typ {
                            Types::sprite => match sprtyp {
                                SpriteTypes::tile => {
                                    is.push(SpriteStruct::new(
                                        &name,
                                        &e.to_string(),
                                        SpriteTypes::tile,
                                    ));
                            ...
                            Types::font => {
                                fs.push(FontStruct::new(
                                    &name,
                                    graphics::Font::new(ctx, e.to_string(), 32)?,
                                ));
                            }
                        }
                        id = id + 1;
                    }
                    Elements::alternate => match typ {
                        Types::sprite => {
                            is.get_mut((&id - 1) as usize)
                                .unwrap()
                                .altsprites
                                .push(e.to_string());
                        }
                ...
                Ok(XmlEvent::StartElement { name, .. }) => match name.to_string().as_ref() {
                    "location" => {
                        elm = Elements::location;
                    }
                    "name" => {
                        elm = Elements::name;
                    }
                    "alternate" => {
                        elm = Elements::alternate;
                    }
                    "sprites" => {
                        typ = Types::sprite;
                    }
                    ...
                    "tiles" => {
                        sprtyp = SpriteTypes::tile;
                    }
                    "actors" => {
                        sprtyp = SpriteTypes::actor;
                    }
                    "buildings" => {
                        sprtyp = SpriteTypes::building;
                    }
                    _ => {}
                },
                Err(e) => {
        ...
        for f in fs {
            assets.add_font(&f.name, f.font).unwrap();
        }
        Ok(assets)
    }
/*
