...
const ZOOMSTEP: [f32; ZOOMLEVELS] = [
    0.3, 0.33, 0.36, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.9, 1.0,
];
...
impl Camera {
    pub fn new(ctx: &mut ggez::Context) -> Self {
        Camera {
            position: Point2::new(
                (MAPSIZE_MAX_X as f32 / 2.0) * -TILESIZE as f32,
                (MAPSIZE_MAX_Y as f32 / 2.0) * -TILESIZE as f32,
            ),
            zoomlevel: 1.0,
            zoomstep: 9,
    ...
    pub fn zoom(&mut self, newzoom: i32, mousepos: Point2) -> bool {
        ...
        if self.zoomstep >= ZOOMLEVELS as i32 {
            self.zoomstep = ZOOMLEVELS as i32 - 1;
            return false;
        }
        ...
        self.maplimx =
            -((MAPSIZE_MAX_X as f32 * self.tsize) as i32 - self.scrX + (self.tsize) as i32);
        self.maplimy =
            -((MAPSIZE_MAX_Y as f32 * self.tsize) as i32 - self.scrY + (self.tsize) as i32);
    ...
    pub fn movestep(&mut self, x: f32, y: f32) -> bool {
        let p = Point2::new(
            self.position.x + (x * MOVESTEP),
            self.position.y + (y * MOVESTEP),
        );
