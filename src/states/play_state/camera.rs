...
const ZOOMLEVELS: usize = 4;
const ZOOMSTEP: [f32; ZOOMLEVELS] = [0.3, 0.8, 1.0, 1.3];
...
/// Returns screenspace coordinates (x, y) of center of specified game tile
  ...
  pub fn zoom (&mut self, newzoom: i32, mousepos: Point2) -> bool {
    ...
    let offsetX = ((-(self.scrX / 2) as f32 + mousepos.x) / self.tsize) as i32;
    let offsetY = ((-(self.scrY / 2) as f32 + mousepos.y) / self.tsize) as i32;
    ...
    self.min_x = x;
    self.min_y = y;
    self.movetotile(mx - offsetX, my - offsetY);
    true
  }
